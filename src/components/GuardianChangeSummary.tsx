...
const GuardianSide: FC<GuardianSideProps> = ({ label, endpoint }) => {
  ...
  return (
    <div data-testid="guardian-change-side" className="flex min-w-0 flex-1 flex-col items-center gap-2 text-center">
...
/**
 * A guardian change, read left to right: the provider being left, an arrow, the provider taking
 * over — each on the same 48px brand tile the guardian picker and Guardian settings draw, so a
 * provider looks the same everywhere. `single` is the device-key rotation, which changes no
 * provider: the guardian is drawn once and the key itself belongs in the detail card below.
 */
export const GuardianChangeSummary: FC<GuardianChangeSummaryProps> = props => {
  ...
  return (
    <div
      ...
      className={cn('flex w-full items-start justify-center gap-3 rounded-2xl bg-fill px-4 py-4', className)}
    >
      {props.kind === 'single' ? (
        ...
        <>
          ...
          {/* Decorative: the From/To labels already say which way this reads. `mt-3.5` centers the
              20px glyph on the 48px tile beside it. */}
          <Icon
            ...
            className="mt-3.5 shrink-0 text-muted"
          />
