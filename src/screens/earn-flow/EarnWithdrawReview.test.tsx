...
describe('EarnWithdrawReview', () => {
  ...
  it('renders the selected position and the full-withdraw route details', () => {
    ...
    expect(screen.getByText('Full position (gasless)')).toBeInTheDocument();
  ...
  it('rejects a position not owned by the current wallet account', async () => {
    ...
    expect(
      await screen.findByText('This position is not owned by the current wallet-derived EVM account.')
    ).toBeInTheDocument();
  ...
  it('uses the fallback message for non-Error failures', async () => {
    ...
    expect(await screen.findByText('Gasless withdrawal failed.')).toBeInTheDocument();
  });
