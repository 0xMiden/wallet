...
export const SelectRecipient: React.FC<SelectRecipientProps> = ({
  ...
  const titleChip = !isValidAddress ? undefined : isEthereum ? (
    <NetworkChip kind="ethereum" label={selectedNetwork?.name ?? t('ethereum')} data-testid="send-recipient-network" />
  ) : (
  ...
  return (
    ...
    >
      ...
      {isValidAddress && isEthereum && (
        <div className="mt-4 flex flex-col gap-2" data-testid="send-network-options">
