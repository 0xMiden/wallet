...
describe('ReviewSwap', () => {
  ...
  describe('rate row', () => {
    it('shows no rate and uses the fallback ETA when no quote is available', () => {
      renderComponent({ swapEta: undefined });

      const [row, etaRow] = screen.getAllByTestId('review-row');
      expect(row).toHaveAttribute('data-label', 'rate');
      expect(row).toHaveTextContent('');
      expect(etaRow).toHaveAttribute('data-label', 'usuallyFillsIn');
      expect(etaRow).toHaveTextContent('swapEtaFallback');
    });

    it('renders the oracle market rate and next-batch ETA for fillable orders', () => {
      renderComponent({
        swapEta: {
          canFill: true,
          estimatedSeconds: 45,
      ...
      expect(values[0]).toHaveTextContent('1 IMIDEN ≈ 2 IETH');
      expect(values[1]).toHaveTextContent('swapEtaSeconds');
    });

    it('uses the 24-hour median when the order cannot fill immediately', () => {
      renderComponent({
        swapEta: {
          canFill: false,
          estimatedSeconds: null,
          offMarket: false,
          marketPrice: '0.333333',
          median24hSeconds: 120
        }
      });
