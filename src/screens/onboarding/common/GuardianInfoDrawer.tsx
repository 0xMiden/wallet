...
const InfoRow: React.FC<InfoRowProps> = ({ icon, title, description }) => (
  <div className="flex flex-col p-4 rounded-xl bg-gray-50">
    <div className="flex items-center gap-3">
      {icon}
      <h3 className="text-base font-semibold text-heading-gray">{title}</h3>
    </div>
    <p className="mt-2 text-sm text-grey-600 leading-[140%]">{description}</p>
  </div>
);
      ...
      <DrawerContent className="max-h-[90vh]">
        <div className="flex flex-col overflow-y-auto px-6 pb-4">
          <div className="flex items-start justify-between">
            <h2 className="text-2xl font-semibold text-heading-gray leading-tight tracking-tight">
              {t('whatIsAGuardian')}
            </h2>
            <button
              type="button"
              ...
              className="shrink-0 w-9 h-9 rounded-full bg-gray-50 flex items-center justify-center"
            >
