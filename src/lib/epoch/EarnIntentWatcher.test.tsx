...
const withdrawRow = (extraInputs: Partial<IEarnDepositExtraInputs & IEarnWithdrawExtraInputs>): Row => ({
  id: 'wd-1',
  type: 'earn-withdraw',
  status: COMPLETED,
  extraInputs
...
describe('EarnIntentWatcher', () => {
  ...
  it('ignores rows restored from a backup', async () => {
    mockRows = [
      { ...depositRow({ epochStatus: 'pending', intentNonce: 'N1' }), restoredFromBackup: true },
      { ...withdrawRow({ phase: 'delivering', withdrawIntentNonce: 'N2' }), restoredFromBackup: true }
    ];
    render(<EarnIntentWatcher />);
    await settle();
    expect(mockReconcileEarnDeposits).not.toHaveBeenCalled();
