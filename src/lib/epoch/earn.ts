...
export const MIDEN_USDC_FAUCET = '0x0a7d175ed63ec5200fb2ced86f6aa5';
...
export async function openEarnPosition(args: OpenEarnPositionArgs): Promise<{ txId?: string }> {
  ...
  if (earnTxId && intent.intentNonce) {
    await updateEarnDepositStatus(earnTxId, 'pending', { intentNonce: intent.intentNonce }).catch(err =>
