...
export function SwapOrderTrackingManager(): null {
  ...
  useEffect(() => {
    ...
    void tick();
    const timer = setInterval(() => void tick(), BASE_INTERVAL_MS);
    return () => {
      disposed = true;
      clearInterval(timer);
    };
...
async function findPollableOrders(): Promise<PollableOrder[]> {
  ...
  const swaps = await Repo.transactions.filter(tx => tx.type === 'swap' && tx.extraInputs?.orderId != null).toArray();
...
async function pollOrder(orderId: string, schedule: SwapOrderSchedule): Promise<void> {
  ...
  setEntry(orderId, { tracking: previous?.tracking ?? null, loading: true });
