...
export function doSync(force = false): Promise<void> {
  if (inFlight) {
    ...
    // A banner retry can arrive after syncState has failed but while runSync
    // is still publishing cached notes/balances. Joining that doomed run would
    // make the click a no-op, so coalesce forced callers onto one follow-up
    // probe that starts as soon as the current run fully settles.
    if (!queuedForcedSync) {
