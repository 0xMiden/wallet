...
export interface GuardianHistoryRecoveryContext {
  createClient: (account: WalletAccount, endpoint: string) => Promise<{
...
export async function recoverGuardianHistory(account: WalletAccount, context: GuardianHistoryRecoveryContext) {
  ...
  previous.push(...Object.values(initialState.checkpoints)
  ...
  try {
    for (const operator of operators) {
      ...
      let checkpoint: GuardianHistoryCheckpoint = initialState.checkpoints[id] ?? {
        id, network, accountId: canonicalAccountId, operator,
        version: GUARDIAN_HISTORY_VERSION, seenCursors: [], completed: false, restored: 0
      };
      ...
      try {
        ...
        while (!checkpoint.completed) {
          await reportGuardianNoteRecoveryProgress({ accountId: account.publicKey, step: 'history', operator, restored });
          const page = await historyRequest(
            () => guardian.getDeltaHistory(guardianAccountId, { limit: 50, cursor: checkpoint.cursor }), check
          );
          if (page.entries.length > 50) throw new GuardianHistoryDataError('Guardian history page exceeds the requested limit');
          if (page.nextCursor && (page.nextCursor === checkpoint.cursor || checkpoint.seenCursors.includes(page.nextCursor))) {
          ...
          for (const entry of page.entries) {
            ...
            await check();
            let summary: GuardianSummary | undefined;
            try {
              summary = await midenClientProxy.decodeGuardianHistory(delta.deltaPayload.txSummary.data);
            } catch (error) {
              if (error instanceof OperationAbortedError || error instanceof WasmClientPoisonedError) throw error;
              console.warn('[GuardianHistory] Summary is unavailable:', error);
            }
            const record = recoveredHistoryRecord(account.publicKey, canonicalAccountId, network, operator, entry, delta, summary);
            records.push(record);
          }
            ...
            for (const record of records) {
              ...
              const match = existing.find(row => {
                ...
                return inputMatches || outputMatches ||
                  (row.type === 'consume' && record.type === 'consume' && sameNonemptyNotes(row.noteIds, record.noteIds));
              });
              if (match) {
                if (match.recovery) {
                  const operators = normalizeHistoryOperators([...match.recovery.operators, operator]);
                  const recovery = { ...match.recovery, operators };
                  if (match.recovery.completeness === 'partial' && record.recovery?.completeness === 'decoded') {
                    await transactions.put({ ...record, id: match.id, recovery: { ...record.recovery, operators } });
                  } else {
          ...
          checkpoint = {
            ...
            completed: !page.nextCursor, restored: checkpoint.restored + added, failure: undefined
          };
          if (!await saveGuardianHistoryCheckpoint(initialState.generation, checkpoint)) throw new HistoryInterrupted();
        }
      } catch (error) {
        if (error instanceof HistoryInterrupted || error instanceof OperationAbortedError || error instanceof WasmClientPoisonedError) throw error;
        sourceFailures++;
    ...
    if (error instanceof HistoryInterrupted || error instanceof OperationAbortedError || error instanceof WasmClientPoisonedError) {
