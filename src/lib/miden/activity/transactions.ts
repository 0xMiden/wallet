...
export const initiateConsumeTransaction = async (
  ...
  return initiateConsumeNotesTransaction(accountId, [note], delegateTransaction, manualRetry, background);
};
...
export const initiateConsumeNotesTransaction = async (
  ...
  delegateTransaction?: boolean,
  manualRetry?: boolean,
  // Background/auto-consume: cold-key routed on Guardian accounts (no biometric).
  // Only the single-note wrapper sets this; user-initiated batch claims leave it off.
  background?: boolean
): Promise<string> => {
  if (notes.length === 0) {
  ...
  const { committedId, queuedNoteIds } = await Repo.db.transaction('rw', Repo.transactions, async () => {
    ...
    const dbTransaction = new ConsumeTransaction(accountId, queueable, delegateTransaction, background);
...
const generateGuardianTransaction = async (
  ...
  switch (transaction.type) {
    ...
    case 'consume': {
      ...
      // Batch claims (Claim All / Claim Group) consume every note in one proposal.
      proposalResult = await withGuardianConflictRetry(() =>
        service.createConsumeNotesProposal(consumeTx.noteIds ?? [consumeTx.noteId])
      );
      break;
    }
