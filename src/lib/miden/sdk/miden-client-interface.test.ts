describe('MidenClientInterface', () => {
  ...
  // Mobile multi-threaded WASM readiness gating.
  //
  // SKIPPED until Phase 2 of the mobile-MT plan lands the gate change at
  // miden-client-interface.ts:517 (see /Users/celrisen/.claude/plans/
  // precious-twirling-parasol.md, "Phase 2 step 2"). At that point the
  // gate becomes:
  //   const mt = await ensureMtReady();
  //   const mobileMtBroken = isMobile() && !mt.mtReady;
  //   const shouldDelegate = delegateTransaction || mobileMtBroken;
  //
  // Today on `next` the gate is `isMobile() ? true : delegateTransaction`
  // (the always-delegate-on-mobile override), so these cases would all
  // delegate regardless of `mtReady`. Skipping until the gate change
  // activates them — flipping `.skip` to live then is the load-bearing
  // evidence that mtReady is wired through.
  //
  // Covers all four cells of the (delegateTransaction × mtReady) matrix
  // for the mobile path. Chrome is unaffected (the `isMobile()` short-
  // circuit in mobileMtBroken means mtReady is never consulted there).
  // eslint-disable-next-line jest/no-disabled-tests -- intentionally skipped pending Phase 2 gate change; flip to live when the gate at miden-client-interface.ts:517 consults ensureMtReady (see plan).
  describe.skip('mobile MT readiness gating (activates with Phase 2 of mobile-MT plan)', () => {
    // Helper: configure isMobile + ensureMtReady mocks, build a fake
    // client with both prove paths instrumented, run a consumeNoteId,
    // assert which prover was used. `consume` is invoked once on the
    // delegate path (no prover arg) and once on the local path
    // (TransactionProver.newLocalProver() as arg).
    async function runMobileGateCase(opts: {
      delegateTransaction: boolean;
      mtReady: boolean;
    }): Promise<{ usedLocalProver: boolean }> {
      ...
      jest.doMock('./init-mobile-mt', () => ({
        ensureMtReady: jest.fn(async () =>
          opts.mtReady ? { mtReady: true, threads: 4 } : { mtReady: false, reason: 'init-failed' }
        )
      }));
      jest.doMock('@miden-sdk/miden-sdk', () => ({
        TransactionProver: { newLocalProver: jest.fn(() => 'local-prover-instance') }
      }));
      ...
      await client.consumeNoteId({
        ...
        noteId: 'note-1',
        type: 'consume',
        delegateTransaction: opts.delegateTransaction
      } as any);

      // The first call's arguments tell us which prover was passed.
      // Delegate path passes undefined (or no second arg);
      // local path passes a TransactionProver instance.
      const firstCallArgs = consume.mock.calls[0];
      const proverArg = firstCallArgs[firstCallArgs.length - 1];
      const usedLocalProver = proverArg === 'local-prover-instance';
      return { usedLocalProver };
    }

    it('delegateTransaction=true + mtReady=true → delegates (user choice wins)', async () => {
      const { usedLocalProver } = await runMobileGateCase({ delegateTransaction: true, mtReady: true });
      expect(usedLocalProver).toBe(false);
    });

    it('delegateTransaction=true + mtReady=false → delegates (user choice wins)', async () => {
      const { usedLocalProver } = await runMobileGateCase({ delegateTransaction: true, mtReady: false });
      expect(usedLocalProver).toBe(false);
    });

    it('delegateTransaction=false + mtReady=true → uses local prover (the headline new behavior)', async () => {
      const { usedLocalProver } = await runMobileGateCase({ delegateTransaction: false, mtReady: true });
      expect(usedLocalProver).toBe(true);
    });

    it('delegateTransaction=false + mtReady=false → force-delegates (the bug Codex flagged)', async () => {
      // Without this branch, the gate would call newLocalProver() on a
      // WASM client whose rayon pool never initialised → crash. The
      // mobileMtBroken clause in the Phase 2 gate change closes that hole.
      const { usedLocalProver } = await runMobileGateCase({ delegateTransaction: false, mtReady: false });
      expect(usedLocalProver).toBe(false);
    });
  });
