...
free note to whichever row ran first. A consume row tagged for either flow (`midenNameClaim`
or `midenNameReturn`) is linked through `linkedRowIdOf`, so a return note is never claimed as
...
`initiateConsumeTransactionFromId` and `tagConsumeAsMidenNameClaim`/`Return`). The tracker
ADOPTS it (`DeliveryNoteState` `untagged`): it writes the tag, and moves the row to
`claiming`/`returning` (live consume) or straight to `owned`/`done` (Completed consume, whose
completion hook could not move the row without the tag). Without this a registration stayed
`issued` for ever while the account held the name, and the scan cursor passed the note.
...
no record, else `pending` with a Publish button. Publish (status page and Settings) asks first in
the wallet's confirm sheet (`useConfirm`) that a publish reveals the owner. The Publish button waits for the first record read (`record !== 'checking'`), so
