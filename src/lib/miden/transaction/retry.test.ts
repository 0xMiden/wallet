...
describe('requeueFailedTransaction — double-send idempotency guard', () => {
  it('does NOT resubmit a send the node reports as landed; completes it instead', async () => {
    ...
    // Completed, NOT reset to Queued — no second send is broadcast. The row is
    // written directly rather than through `updateTransactionStatus`, which
    // refuses to re-finalize the Failed row this function always operates on
    // (see retry.landed.test.ts, which runs the real helper + row store).
