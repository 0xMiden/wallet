...
export class AuthDigest {
  static fromAccountIdWithTimestamp(accountId: string, timestamp: number): Word {
    ...
    const feltArray = new FeltArray([prefix, suffix, new Felt(BigInt(timestamp)), new Felt(0n)]);
  ...
  private static fromAccountIdTimestampAndPayloadWord(accountId: string, timestamp: number, payloadWord: Word): Word {
    const paddedHex = accountId.startsWith('0x') ? accountId : `0x${accountId}`;
    const parsedAccountId = AccountId.fromHex(paddedHex);
    const prefix = parsedAccountId.prefix();
    const suffix = parsedAccountId.suffix();

    const feltArray = new FeltArray([prefix, suffix, new Felt(BigInt(timestamp)), ...payloadWord.toFelts()]);
