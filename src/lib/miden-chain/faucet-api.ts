...
export async function getPowChallenge(
  ...
  const response = await fetch(`${baseUrl}/pow?${params}`, { signal });
...
export async function solvePowChallenge(challengeHex: string, target: bigint, signal?: AbortSignal): Promise<number> {
...
export async function requestTokens(
  ...
  const response = await fetch(`${baseUrl}/get_tokens?${params}`, { signal });
