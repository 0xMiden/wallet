/**
 * Wrapper-level test for the native hot-key path. This does NOT validate
 * byte-for-byte ECDSA wire-format parity against the WASM SDK — that gate is
 * the manual on-device run. What we do guard here:
 *  - iOS and Android both forward generate / sign / delete to the HotKey
 *    plugin (same wire format on both platforms, see HotKeyPlugin.swift /
 *    HotKeyPlugin.kt).
 *  - Calling outside iOS/Android (e.g. extension/desktop) throws so callers
 *    can't accidentally hit a no-op native bridge instead of the JS fallback.
...
const sharedForwardingSpec = (label: 'iOS' | 'Android', setPlatform: () => void) => {
  describe(`secure-hot-key nativePlugin (${label})`, () => {
...
sharedForwardingSpec('iOS', () => {
  mockIsIOS.mockReturnValue(true);
  mockIsAndroid.mockReturnValue(false);
});

sharedForwardingSpec('Android', () => {
  mockIsIOS.mockReturnValue(false);
  mockIsAndroid.mockReturnValue(true);
});

describe('secure-hot-key nativePlugin (non-mobile guard)', () => {
  // Off-mobile (extension/desktop) should never hit this module — secure-hot-key
  // routes to jsFallback there. Belt-and-braces: every export throws so a
  // misrouting is loud rather than a silent no-op.
  it.each([
