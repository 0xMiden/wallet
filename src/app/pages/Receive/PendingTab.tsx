...
interface PendingTabProps {
  ...
  /**
   * Currently expanded asset group, owned by the host page so its header back
   * affordance can close the detail view before leaving the page.
   */
  selectedFaucetId: string | null;
  onSelectFaucetId: (faucetId: string | null) => void;
...
export const PendingTab: React.FC<PendingTabProps> = ({
  ...
  useEffect(() => {
    if (selectedFaucetId && !selectedGroup) {
      onSelectFaucetId(null);
    }
  }, [selectedFaucetId, selectedGroup, onSelectFaucetId]);

  const handleSelectGroup = useCallback(
    (faucetId: string) => {
      hapticLight();
      onSelectFaucetId(faucetId);
    },
    [onSelectFaucetId]
  );
  ...
  // Hardware / swipe back on mobile closes the detail view before the page.
  useEffect(() => {
...
const PendingSummary: React.FC<PendingSummaryProps> = ({
  ...
  if (groupedNotes.length === 0) {
    return (
      <div className="flex-1 min-h-0 overflow-y-auto">
        <div className="w-full mx-auto py-4 flex flex-col min-h-full">
  ...
  return (
    <div className="flex-1 min-h-0 overflow-y-auto">
      <div className="w-full mx-auto py-2 flex flex-col min-h-full">
...
const AssetPendingDetail: React.FC<AssetPendingDetailProps> = ({
  ...
  return (
    <div className="flex-1 min-h-0 overflow-y-auto">
      <div className="w-full mx-auto pt-6 flex flex-col min-h-full">
