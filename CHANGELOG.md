...
* [FIX][all] Dark-mode contrast regressions across onboarding, send flow, settings, and shared layouts. Several call sites used the literal `grey-*` palette (mapped to fixed light-grey hex values in `src/utils/tailwind-colors.js`) or literal `#FFFFFF` / `#F2F2F2` instead of the auto-flipping theme tokens (`bg-gray-*`, `bg-input-bg`, `text-text-muted`) defined as CSS vars in `src/main.css`. Result: `Button.Ghost` and `Chip` hovers painted white-on-white in dark mode (Welcome / BackupSeedPhrase / VerifySeedPhrase); the Private Payment unselected "On" button (SendDetails) was white-on-white; the Review Transaction toggle's "Off" labels were invisible. Also fixed `PageLayout`'s toolbar Close button — it passed a literal `fill="black"` SVG color instead of `fill="currentColor"`, so the X stayed black on the dark wrapper across every page (HistoryDetails, Send subroutes, etc.). (closes #202, #219)
* [FIX][all] Privacy Policy and Terms of Use links now resolve. The wallet pointed at `https://www.miden.fi/{terms,privacy}` (Welcome onboarding's CreatePassword screen + Settings → About) but the entire `miden.fi` domain is dead — every URL 404s. The About-page entries additionally used `slug: '#'` with `linksOutsideOfWallet: true`, which renders `<a href="#">` and just scrolled the popup to top. Both surfaces now share `PRIVACY_POLICY_URL` / `TERMS_OF_USE_URL` constants in `src/app/constants.ts` pointing at `https://miden.xyz/privacy`. **Stopgap:** there is currently no Terms page on `miden.xyz` (every variation 404s), so `TERMS_OF_USE_URL` temporarily points at the Privacy page until a real Terms page ships. Inline comment in `constants.ts` flags the gap. (closes #202, #219)
* [FIX][extension] Advanced Settings now displays the account public key. The page fetched `getPublicKeyCommitments()` and stored the value only in a hidden `<input className="sr-only">` for clipboard copy, so users saw a blank label. Added a truncated `0x{first6}...{last4}` display next to the "Account Public Key" label and disabled the Copy button while the value is loading. (closes #219)
