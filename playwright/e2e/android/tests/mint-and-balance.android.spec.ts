...
test.describe('Faucet Minting and Balance (Android)', () => {
  ...
  test('deploy faucet and mint tokens to both wallets', async ({
    ...
    // Android divergence from Chrome (same as iOS): no chrome.storage.local
    // for pending notes, so claim explicitly before checking balance. The
    // claim is what exercises the native Rust prover via the
    // MidenNativeProver Capacitor plugin.
