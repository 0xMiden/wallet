...
test.describe('earn: withdrawal recovery', () => {
  ...
  test('recovers exact allocations after the submitting owner closes before the relay answers', async ({
    ...
    try {
      ...
      allocator.setIntentStatus(evmOwner, '22', [{ status: 'pending', chainId: MIDEN_CHAIN_ID, transactionHash: '' }]);
      ...
      expect(received.submissionState, 'received delivery must not acknowledge the rejected sibling').toBe('prepared');
      const receipt = received.receipts.find(candidate =>
        candidate.noteIds.some(id => noteKey(id) === noteKey(noteId))
      );
      if (!receipt?.transactionId || !receipt.midenNoteId) throw new Error('Real completed consume receipt is missing');
