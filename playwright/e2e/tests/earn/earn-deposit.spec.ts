...
function defineEarnDepositSuite(axis: AccountAxis): void {
  test.describe(`earn: deposit happy path - ${axis.label} account`, () => {
    ...
    test('deposit USDC collateral → earn-deposit row completes and epoch confirms', async ({
      ...
      if (axis.label === 'guardian') test.setTimeout(600_000);
...
defineEarnDepositSuite(offChainAxis);
defineEarnDepositSuite(guardianAxis(getEnvironmentConfig().guardianUrl));
