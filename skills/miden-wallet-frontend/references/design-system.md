...
| List group | `ListGroup` | One of the three list surfaces (see Surfaces), set by `surface`: `plain` for a page that is a list, `fill` (default, 16px radius) for a group embedded in a page or a sheet, `outline` for a card that has to separate itself. Hairlines between rows, inset past the leading visual on `fill` and `outline`, full width on `plain` unless `insetHairlines` keeps each row's own inset (read from the row's `--row-flush-inset`, which `CheckboxRow` and `FactRow` declare) - a checklist or a sheet's facts. `as="ul"` for a list of `li` rows. | ad-hoc stacks, full-page lists drawn as one card |
