...
currency at 3:1, the change pill at 4.5:1 and the footer well (4.47:1 on orange, 4.5:1 elsewhere).
...
| `text-value` | Nunito | 15 / 20 | 700 | `DetailRow` values, Activity row amounts, the balance card's label, a `FactRow` item (the guardian guarantees) |
...
| `text-caption-heading` | Nunito | 13 / 17 | 600 | A fact's body under its title: `FactRow` and `CheckboxRow` descriptions (the testnet notice, the Guardian explainer, the account-setup checklist), and the guardian step's operator bio |
...
| List group | `ListGroup` | One of the three list surfaces (see Surfaces), set by `surface`: `plain` for a page that is a list, `fill` (default, 16px radius) for a group embedded in a page or a sheet, `outline` for a card that has to separate itself. Hairlines between rows, inset past the leading visual on `fill` and `outline`, full width on `plain` unless `insetHairlines` keeps each row's own inset (read from the row's `--row-flush-inset`, which `CheckboxRow` and `FactRow` declare) - a checklist or a sheet's facts. `as="ul"` for a list of `li` rows, carrying `role="list"` (WebKit drops list semantics from a marker-less `ul`). | ad-hoc stacks, full-page lists drawn as one card |
| Fact row | `FactRow`, `IconCircle` (`components/ui/FactRow`) | `IconCircle`: the round `fill` disc a leading glyph sits in, `aria-hidden`; `md` 32px with a 16px glyph, `sm` 20px leaving the glyph its size; a caller's tint class replaces the fill. `FactRow`: a row whose text wraps, which `ListRow`'s one-line subtitle cannot: `fact` puts a `text-row-title` title over a `text-caption-heading` `muted` description after a 32px circle, `item` one `text-value` line after a 20px circle; the hairline above every row but the first starts after the circle. | hand-built fact rows and icon circles |
...
| Text action | `TextAction` | `text-action` `accent-tint-ink`, no underline, 44px hit area, tap haptic: "Learn more", "Use a custom URL". `layout="row"`: a full-width action row closing a section, the label left and a chevron right, a hairline under it, 48px tall ("Choose a different Guardian"). | underlined `primary-500` links, hand-styled text buttons |
...
| Sheet | `Drawer` (vaul) | 28px top corners, 36 × 5 handle, 18px title left, 32px ✕ right, 16px margin, no rule under the header; choices in `ListGroup`s on `fill`, facts as plain `FactRow`s (`insetHairlines`); mobile back closes an open sheet through the shared `Drawer`, unless its host's page handler owns that and passes `closeOnBack={false}` (the send and swap pickers); one decision per sheet; CTA pinned. Opens and closes on the tab-bar springs over a plain `scrim` (see Motion). | `CustomModal`, `ModalWithTitle`, custom overlays; the react-modal dependency goes last |
