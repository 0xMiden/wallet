...
| `text-title-tab` | Nunito | 28 / 36, −0.5px | 800 | Page titles: `TabHeader`, `PageHeader` and `FlowLayout` step titles |
...
| List group | `ListGroup` | `surface` `fill` (default): 16px radius on `fill`, hairlines between rows inset past the leading visual. `outline`: the same shape as a hairline edge on `page`. `plain`: no surface, rows flush on the page margin with full-width hairlines. | ad-hoc stacks |
