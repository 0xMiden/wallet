...
The lock self-recovers from a wedged holder (issue #775 — a WASM trap abandons its promise without settling, so `finally` never releases): a realm `error`/`unhandledrejection` listener evicts the holder on a trap in milliseconds, and a 5-minute watchdog (`WASM_LOCK_WATCHDOG_MS`, `sdk/wasm-client-poison.ts`) bounds any other silent wedge. Both reject the holder with `WasmClientPoisonedError` and replace the client singletons so the next `getMidenClient()` builds a fresh client. An EVICTION never frees the old instance on the spot — the evicted flow is past the mutex, still holds that reference and is by construction inside a WASM call, so freeing would fail a write that may already have submitted; it is marked poisoned instead (its `isDisposed` guards fire) and freed later, once EVERY flow retaining that instance has settled — the abandoned holder, any sibling suspended mid-yield, and the mutex owner that resolved the same instance — since any of them may still be writing. The same rule governs the one path that discards a client for a reason other than a trap, an endpoint-change `resetMidenClient()`, because it takes nothing a yielded holder holds: it marks and defers while any flow is suspended mid-yield rather than freeing under it. Nothing else replaces the client: since #878 each realm keeps ONE `MidenClientInterface` for every read and write, and its SDK keystore routes to callbacks the realm installs once (`installRealmKeystore`): `Actions.init` installs the vault signer in every realm that writes (the SW, mobile, desktop; the extension popup never runs it and never signs), a vault installs its insert-key sink when it takes its key, the realm's sink is re-derived from the store when the flow that constructed the vault ends (`syncRealmInsertKeySink` in actions.ts, adopted or failed), and lock retires only the vault it locks, so a spawn's sink survives a lock landing mid-flow; a hold in which an already-adopted vault inserts key material (`createHDAccount`, `importAccountFromPrivateKey`) asserts first that the sink is its own and refuses as locked otherwise, so a lock that landed while the create or import waited on the accounts queue fails it before any irreversible step; a spawn's hold does not assert, since its sink is the one its own constructor just installed and a lock retires only the store's vault. So `getMidenClient()` takes no options and is never rebuilt for a call; a sign reaching a replaced client is refused with `WasmClientPoisonedError`, an insert reaching one lands only while the installed insert-key sink is the one that client's build was retired against, judged by a per-sink id the build's trampolines carry so no closure is retained (the SDK has persisted the account by the time it asks; a secret under another vault's key is worse than none), and a call made before the realm installed the callback fails with a named error. A locked vault reported by a sign is recorded under the lock HOLD the sign ran for, and `withWasmClientLock` carries it out on that hold's own rejection (`tagLockedSignReason`, the same tag the offscreen path sets per op); the transaction loop reads only the error, never an ambient slot (a realm-wide one let a dry run's locked sign requeue an unrelated write). What that CANNOT reclaim is an operation that never settles at all: a gRPC-web fetch the node accepted and will never answer holds its client for the realm's lifetime, so the fuse (not the deferred free) is what bounds how many of those a parked node can accumulate. Only a trap with no holder at all disposes immediately.

A JS timeout is NOT a substitute for the ceiling: rejecting out of a lock callback releases the mutex while the abandoned call keeps running, which is safe only when the WASM is in another realm behind its own mutex. `runsWasmInThisRealm()` (`back/miden-client-proxy.ts`) is how a caller tells the difference — `sync-manager` keeps its 30s timeout on the offscreen path and takes `watchdogMs` on the inline one.

If a lock-held flow legitimately waits unboundedly (user-facing sign, local prove), bracket that wait in `withWasmLockWatchdogPaused`, passing the hold. The pause RELAXES the ceiling to `WASM_LOCK_PAUSED_WATCHDOG_MS` (30 min) rather than stopping the clock — a stopped clock made the backstop optional, which is the pre-#775 wedge reached through the fix's own escape hatch. Both ledgers are per-HOLD, not per-bracket: paused time is banked in `pausedElapsedMs` (sequential brackets share the 30 min), running time in `unpausedElapsedMs`, and the close re-arms only the unspent normal budget plus one 30 s finishing slice if it already ran out. So it is NOT the same shape as the proxy's `pauseDeadline`/`resumeDeadline`, which does restart from scratch: no loop of brackets can buy unlimited unwatched time. A yielded wait (`yieldWasmClientLock`) draws on the same relaxed ledger — it banks its wall-clock into `pausedElapsedMs` on the way out, so alternating a sign pause and a prove yield cannot buy 30 minutes twice — and a yield that never settles is evicted, not wedged. Both ledgers grant ONE `WASM_LOCK_MIN_WATCHDOG_MS` finishing slice when a legitimately-unbounded wait exhausts the budget (`graceUsed`, `pausedGraceUsed`); without the paused one, a 30-minute sign prompt left the prove that follows it arming at a ceiling of zero.

Every bounded hold passes a `label` in its lock options; both eviction logs print it as `hold`. A dozen sites now share the two-minute sync ceiling, so an unlabelled record cannot say which flow parked.

The realm's sync FUSE (`lib/miden/front/sync-fuse.ts`) is module-scoped and shared, NOT private to `useSyncTrigger`: what it records is a fact about the realm's parked client, and four timers drive a hold against that one client. But it is KEYED PER PROBE (`SyncFuseKey`: `idle-sync`, `claimable-notes`, `balances`, and `guardian-sync:<accountPublicKey>` via `guardianSyncFuseKey` — otherwise the same strings as the hold `label`s), and that is load-bearing, not cosmetic. One shared counter was defeated by ordering alone: `useSyncTrigger` reports its chain sync's success and only THEN fires guardian sync, so against a healthy node with a parked guardian the count oscillated 0 → 1 → 0 and the threshold was unreachable — the exact every-two-minutes park and per-lap client leak the sharing was added to end. Evidence is withdrawn only by a success on the SAME key.

A KEY IS THE NARROWEST THING THAT CAN PARK INDEPENDENTLY — which is why guardian carries the account. Sharing one `guardian-sync` key reproduced that same defeat one level down: `syncGuardianAccounts` loops the accounts sequentially and each has its own endpoint, so a healthy sibling's success erased the parked account's increment within the same lap. Too coarse and the threshold is unreachable; too fine and no key accumulates enough. Conversely, the swap-settlement tick deliberately SHARES `claimable-notes`: same notes, same proxy call, same endpoint, so the two cannot park independently.

Feeding the ledger is not enough on its own: a probe with no scheduler of its own to stretch must also CONSULT it (`isSyncFused(key)`), or a lit fuse buys nothing. Four do — `syncGuardianAccounts` (gated PER ACCOUNT inside its own loop, next to the 429 cooldown, NOT at the caller: it has two callers, and a caller-side gate covered the mobile/desktop loop while the extension's post-`SyncRequest` trigger went on parking), the claimable-notes SWR poll (via `isPaused`, so the displayed list survives), `fetchBalances` (returns `null`, the same contract as its mutex-busy skip) and `settleSwapOrders` (returns an empty result, which is what "nothing to settle this lap" already looks like). Adding a new timer-driven WASM hold means: bound it, label it, pick a key, report success/eviction/other-failure, and gate it.

Report the success ONLY ONCE THE WHOLE PROBE IS THROUGH. `getClaimableNotes` takes two holds (note read, then swap-lineage classification) and the mutex is released between them, so a success booked after the first cleared the evidence the second was accumulating — the same erasure as a shared key, inside one function.

Every fuse conclusion is about ONE node, so an endpoint change discards all of them — `clearSyncFuseForEndpointChange()` from Developer Settings' save (the frontend realm) and `resetSyncBackoffForEndpointChange()` from the SW's `ReloadEndpointOverridesRequest` handler, before `resetMidenClient()`. Without it a fused wallet repointed at a working node stays near-silent for another 30 min and withholds the successful sync that is the fuse's only exit.

The service worker keeps its own copy of the same rule in `sync-manager.ts`, counted per failed probe rather than per breaker trip — that path zeroes `consecutiveSyncFailures` whenever a window opens, so a counter inside the breaker block would need three times the evidence at two parked minutes each.
...
### Balance loading
`fetchBalances` reads the account from IndexedDB via `getAccount()` under the WASM lock and never syncs; don't call `syncState()` from the UI path. Sync is separate: on mobile and desktop `useSyncTrigger` runs it every 3 s (in-process, under the lock, with backoff and the sync fuse); on the extension the service worker's lap writes `miden_sync_data`, which the popup turns into balances (`updateBalancesFromSyncData`). A refresh takes the lock with a non-blocking try and skips when it is busy, so it never stalls behind a write. Every reader (the Ready-time read in `syncFromBackend`, the `useAllBalances` poll) goes through the store's `fetchBalances` action, the one implementation of a balance read. It passes `waitForLock: balances[address] === undefined`, so a read with nothing on screen queues FIFO: after an import the sync and note reads keep the queue full and a skipping read starves (#1123). It holds `fetchingAddresses`, one read in flight per address, publishes fetched metadata through `setAssetsMetadata`, and stores whatever lands whether or not the component that asked is still mounted. Every await it makes while holding the address is bounded (the metadata lookup is raced against one RPC timeout). Only a landed read ends `balancesLoading`; a failed or fused first read leaves Home on its skeleton. `clearStorage` resets the native-asset cache on every create or import and primes its rediscovery at once, because `fetchBalances` awaits the native faucet id before it returns.
...
Work a page does only for display (a poll, a reconciliation) pauses through `usePageActive()` (`app/layouts/page-active.ts`), which is false in a hidden `TabPane` and in a `PageLayer` that is not the current page; `History` uses it: an SWR read gated this way holds a null key, never `isPaused`, because SWR sends a shared key's mutate and error retry to its first subscriber, and a paused one swallows them. The dApp browser mounts `DappActive` only while its page is on screen, because that unmount is what parks the foreground dApp and hides its native window.

**Slide pages stack on top of the page beneath.** `MobilePageLayers` keeps the covered layer mounted (inert, at -24% with a dim) for as long as a slide `FullScreenPage` is above it, and a pop re-enters that same layer through `AnimatePresence` - no remount, no fade, scroll and state intact. The layer is released only when the page above is not a slide page, or under reduced motion; a webview return keeps it and only makes the motion instant. Do not add per-mount work to a tab page on the assumption that back navigation remounts it.

Whether a navigation goes back is read from the router: a pop, or a navigation to a page in the stack beneath whose layer is still mounted (a close), reveals that page and slides the slide page out, and the page a return leaves is never kept covered. A page kept mounted outside the stack (one a return skipped over, or one a Replace left) is revealed by a router pop but reopened as a new layer by a push, and a page a push reopens always gets a new layer with fresh state. A push from a slide page to a plain page plays no Back animation; it releases the stack, so a later Back remounts the slide page, which then fades in instead of sliding in like a push (a page a slide page's Back reveals plays no entrance of its own).
...
Extension, Capacitor, and Tauri render the same React `BottomNav` from `TabLayout`. `TabLayout` owns destinations, active route state, route changes, haptics, and the `[data-tabbar-footer]` shell; `BottomNav` owns button semantics and presentation. Drawers hide it through `useHideNavbarWhileOpen`, which drives the body data attribute consumed by `src/main.css`. On Capacitor, `public/mobile.html` owns body safe-area padding and `src/main.css` paints the footer strip. On Android `TabLayout` passes `clearInset`, so the docked tabs end 8px above the system navigation bar instead of reaching 16px into the inset as they do over the iOS home indicator, and it marks the body `data-navbar-clears-inset`, from which `src/main.css` sizes the flow CTAs' cushion (`--navbar-cushion`). `DappBrowserProvider` owns embedded dApp WebViews, not wallet navigation; the native navbar API under `packages/dapp-browser` has no wallet frontend caller.
