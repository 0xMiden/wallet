...
The realm's sync FUSE (`lib/miden/front/sync-fuse.ts`) is deliberately module-scoped and shared, NOT private to `useSyncTrigger`: what it records is a fact about the realm's parked client, and more than one loop drives a sync against that one client (`syncGuardianAccounts` takes an equally long hold on the same tick, and its failures are swallowed per account). The service worker keeps its own copy of the same rule in `sync-manager.ts`, counted per failed probe rather than per breaker trip — that path zeroes `consecutiveSyncFailures` whenever a window opens, so a counter inside the breaker block would need three times the evidence at two parked minutes each.
