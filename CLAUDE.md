...
If a lock-held flow legitimately waits unboundedly (user-facing sign, local prove), bracket that wait in `withWasmLockWatchdogPaused`. The pause RELAXES the ceiling to `WASM_LOCK_PAUSED_WATCHDOG_MS` (30 min) rather than stopping the clock — a stopped clock made the backstop optional, which is the pre-#775 wedge reached through the fix's own escape hatch. The close re-arms only the hold's unspent normal budget (running time is banked in `unpausedElapsedMs` across brackets), plus one 30 s finishing slice if that budget already ran out. So it is NOT the same shape as the proxy's `pauseDeadline`/`resumeDeadline`, which does restart from scratch: a loop of brackets here cannot buy unlimited unwatched time.
