...
The realm's sync FUSE (`lib/miden/front/sync-fuse.ts`) is module-scoped and shared, NOT private to `useSyncTrigger`: what it records is a fact about the realm's parked client, and four timers drive a hold against that one client. But it is KEYED PER PROBE (`SyncFuseKey`: `idle-sync`, `guardian-sync`, `claimable-notes`, `balances` — same strings as the hold `label`s), and that is load-bearing, not cosmetic. One shared counter was defeated by ordering alone: `useSyncTrigger` reports its chain sync's success and only THEN fires guardian sync, so against a healthy node with a parked guardian the count oscillated 0 → 1 → 0 and the threshold was unreachable — the exact every-two-minutes park and per-lap client leak the sharing was added to end. Evidence is withdrawn only by a success on the SAME key.

Feeding the ledger is not enough on its own: a probe with no scheduler of its own to stretch must also CONSULT it (`isSyncFused(key)`), or a lit fuse buys nothing. Three do — the fired-and-forgotten `syncGuardianAccounts` call, the claimable-notes SWR poll (via `isPaused`, so the displayed list survives) and `fetchBalances` (returns `null`, the same contract as its mutex-busy skip). Adding a new timer-driven WASM hold means: bound it, label it, add a key, report success/eviction/other-failure, and gate it.
...
An evicted operation is abandoned, not cancelled — treat `WasmClientPoisonedError` like `OperationAbortedError` in any kill classifier (`isWasmClientPoisonedError`), and never assume the pipeline stopped. Concretely: never route a poison error onto a path that requeues a row as a fresh write, because the abandoned pipeline can still submit and the requeue becomes a second payment.
