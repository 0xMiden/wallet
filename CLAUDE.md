...
Fiat ramp: `/buy` is a `FullScreenPage` route (MoonPay, buy only) used on EVERY platform — the widget is a plain remote iframe (`https://buy.moonpay.com`, or `https://buy-sandbox.moonpay.com` for `pk_test_` keys), no SDK, mounted by `src/screens/fiat-ramp/FiatRampScreen.tsx` with the URL built in `src/lib/fiat-ramp/moonpay.ts`. Gated by `isFiatRampEnabled()` (`MOONPAY_API_KEY` baked at build time) plus the account's derived `evmAddress`. Gotcha: MoonPay mandates HMAC-SHA256 signing of URLs carrying `walletAddress` — the signature (base64, over the FULL query string including the leading `?`, appended LAST and URL-encoded) comes from the local sign server (`../moonpay-sign-server.mjs`, port 5567, `MOONPAY_API_SECRET`); any query change after signing invalidates it. The extension iframe needs `credentialless` (COEP `require-corp` + no CORP header from MoonPay) **and** a declarativeNetRequest session rule stripping MoonPay's `frame-ancestors *` CSP (`*` never matches chrome-extension:// origins) — `src/lib/fiat-ramp/moonpay-frame-rules.ts`, installed at SW start.

Post-purchase flow: /buy mount mints a session uuid (`buy-session.ts`, localStorage slot) carried as `externalTransactionId`; the app-root `BuyBridgeManager` (provider.tsx, 20s tick) runs `lib/fiat-ramp/buy-watcher.ts` — polls the sign server's `/tx-status` proxy, and a `completed` purchase inserts a tracking-only `buy` activity row (`BuyTransaction`, born Completed; `extraInputs.bridgeProgress: not-initiated → initiated | failed`). Once the delivered token (`buy-token.ts`, hardcoded Sepolia ERC-20) shows a balance on the derived EVM address, the watcher gasses up via the local paymaster (`../miden-paymaster`, port 5568) and broadcasts approve + `bridgeAsset` (vault-signed, `lib/agglayer/vault-evm.ts`) to the user's Miden account; the AggLayer note auto-consumes on arrival. History renders `buy` rows via the dedicated branch in `HistoryView.buildRowProps`.
