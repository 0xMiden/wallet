...
The lock self-recovers from a wedged holder (issue #775 — a WASM trap abandons its promise without settling, so `finally` never releases): a realm `error` listener evicts the holder on a trap in milliseconds, and a 5-minute watchdog (`WASM_LOCK_WATCHDOG_MS`, `sdk/wasm-client-poison.ts`) bounds any other silent wedge. Both reject the holder with `WasmClientPoisonedError` and dispose the client singletons so the next `getMidenClient()` builds a fresh client. If a lock-held flow legitimately waits unboundedly (user-facing sign, local prove), bracket that wait in `withWasmLockWatchdogPaused` — pause stops the clock, close re-arms the full ceiling from scratch (same shape as the proxy's `pauseDeadline`/`resumeDeadline`). An evicted operation is abandoned, not cancelled — treat `WasmClientPoisonedError` like `OperationAbortedError` in any kill classifier (`isWasmClientPoisonedError`), and never assume the pipeline stopped.
