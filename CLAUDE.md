...
Receive has a Miden | Cross-chain `TabPicker` (shown only when `isDepositAddressBridgeEnabled()` and the account has a derived `evmAddress`); the Cross-chain tab collects amount + token + route. Deep link: `/receive?tab=crosschain&bridge=1&token=ETH` (which now *navigates* to the review page rather than opening a sheet).

**Flow: amount+route on the tab → funding method → wait → review.** `Bridge` on the tab opens `DepositMethodDrawer` (options→wallets/qr) — the last sheet in the flow. Picking any method navigates to **`/deposit-bridge/approve?token=&amount=&method=&wallet=`** (`app/pages/DepositBridge/DepositApprove.tsx`), and detection navigates on to **`/deposit-bridge/review?token=`** (`DepositReview.tsx`). Both are routed `FullScreenPage`s with a `ScreenHeader` — **not** drawer steps; `ApproveInWallet` and `ReviewBridge` are the presentational bodies they render.

- **Approve** restates the request and watches the address, because the wallet cannot know whether the user signed. One CTA, three states: open the wallet → "I have sent the funds" (`claimedSent`, user's word, only nudges `poll()`) → "Funds detected" (`detected`, the chain's word, disabled). All three `DepositApproveMethod`s land here, WalletConnect included — but **WC only reaches it after a session exists; nothing yet pushes the transfer request over that session**, so a WC user still has to send manually.
- **`detected` and `finalised` are different facts, and the gap is shown, not hidden.** `store.detectedArrivals` is populated the moment `detectArrivals` sees money (pre-confirmation); `store.arrivals` only after `CONFIRMATIONS_REQUIRED` stable ticks (~24s). Approve shows step 2 "Confirming on Sepolia" on the first and navigates to review on the second — bridging unconfirmed money is exactly what that wait prevents.
- **An unprompted deposit is picked up automatically.** `DepositArrivalRouter` (`app/templates`, mounted once in `TabLayout`, renders nothing) watches `detectedArrivals` and navigates to the approve page — so money sent to the address from anywhere lands on the same waiting screen, then the review. It routes once per token+balance (a later, larger deposit re-routes) and never fires on `/deposit-bridge/*` or `/generating-transaction/*`.
- **Review** is the only place a bridge is confirmed. **There is no amount step anywhere** — the amount typed on the tab is only the *request*; what gets bridged is `maxSendableDeposit` (arrived balance − gas reserve), so an underpayment bridges the underpaid amount. Its Route row opens the same `RouteCards` and overwrites the saved preference.

The old `DepositBridgeDrawer`, `DepositAmountStep` and `DepositArrivalDrawer` ("funds arrived" sheet, previously mounted in `TabLayout`) are **deleted** — arrival is a navigation now, not a prompt.

**The bridge route is chosen on the tab, not in the drawer.** `CrossChainTab` renders `RouteCards` (extracted from `screens/send-flow/Route.tsx`, which still composes it for the send flow's full-screen route step) under the amount, live-quoting the Epoch fee with the same debounced `quoteDepositViaEpoch` call the drawer used. The choice persists per `${evmAddress}:${token}` via `lib/deposit-bridge/preferred-route.ts` (platform storage, same shape as `watermarks.ts`) — it has to outlive the page, because the bridge only runs once the funds land, possibly after a restart. `DepositBridgeDrawer` reads it back with `readPreferredRoute` (falling back to `getDepositToken(token).route`) and therefore has no route step: its amount step is the last one and confirms the bridge. The `DepositArrivalDrawer` ("funds arrived") is mounted once in `TabLayout` and driven by `useDepositAddressStore.pendingDrawer` (`lib/deposit-bridge`, watcher mounted in `lib/miden/front/provider.tsx`).
