...
### Balance loading
`fetchBalances` reads the account from IndexedDB via `getAccount()` under the WASM lock and never syncs; don't call `syncState()` from the UI path. Sync is separate: on mobile and desktop `useSyncTrigger` runs it every 3 s (in-process, under the lock, with backoff and the sync fuse); on the extension the service worker's lap writes `miden_sync_data`, which the popup turns into balances (`updateBalancesFromSyncData`). A refresh takes the lock with a non-blocking try and skips when it is busy, so it never stalls behind a write. Every reader (the Ready-time read in `syncFromBackend`, the `useAllBalances` poll, the store's `fetchBalances` action) passes `waitForLock: balances[address] === undefined`, so a read with nothing on screen queues FIFO: after an import the sync and note reads keep the queue full and a skipping read starves (#1123). All of them hold `fetchingAddresses`, one read in flight per address, so no reader queues a second hold behind another's. Only a landed read ends `balancesLoading`; a failed or fused first read leaves Home on its skeleton. `clearStorage` resets the native-asset cache on every create or import and primes its rediscovery at once, because `fetchBalances` awaits the native faucet id before it returns.
