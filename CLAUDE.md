...
The lock self-recovers from a wedged holder (issue #775 — a WASM trap abandons its promise without settling, so `finally` never releases): a realm `error`/`unhandledrejection` listener evicts the holder on a trap in milliseconds, and a 5-minute watchdog (`WASM_LOCK_WATCHDOG_MS`, `sdk/wasm-client-poison.ts`) bounds any other silent wedge. Both reject the holder with `WasmClientPoisonedError` and replace the client singletons so the next `getMidenClient()` builds a fresh client — freeing the old one, or (when another flow is suspended mid-yield and still holds a reference) merely marking it poisoned so its `isDisposed` guards fire without failing a write that may already have submitted.
